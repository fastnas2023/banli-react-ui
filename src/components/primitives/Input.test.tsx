...
it('supports controlled value + onChange', async () => {
  ...
  function Controlled() {
    ...
    return <Input aria-label="controlled" value={value} onChange={(e) => setValue(e.target.value)} />
  }

  render(<Controlled />)
  const el = screen.getByLabelText('controlled') as HTMLInputElement
...
it('supports defaultValue (uncontrolled)', async () => {
  const user = userEvent.setup()
  render(<Input aria-label="uncontrolled" defaultValue="hello" />)
...
it('sets aria-invalid when invalid=true', () => {
  render(<Input aria-label="invalid" invalid />)
  expect(screen.getByLabelText('invalid')).toHaveAttribute('aria-invalid', 'true')
})

it('loading implies disabled and sets aria-busy', () => {
  render(<Input aria-label="loading" loading />)
  const el = screen.getByLabelText('loading') as HTMLInputElement
